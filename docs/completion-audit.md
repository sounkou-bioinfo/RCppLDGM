...
1. Full GraphLD graphREML CLI parity: multiprocessing/block-manager behavior,
   richer score-test schema, optimizer/output parity beyond the current two-fixture
   conformance gates, and upstream-scale jackknife conformance.
2. Pinned Python output conformance for broader graphREML optimizer summaries,
   ...
   workloads on upstream GraphLD data. BLUP, clumping, the current GraphREML
   default tiny fixture plus synthetic two-block active-variant fixture, and
   selected/full Hutchinson/xdiag inverse diagonals now have pinned
