...
| GraphLD graphREML score-test HDF5 output | `ldgm_write_score_test_hdf5()`, `ldgm_write_gene_score_hdf5()`, optional `ldgm_run_reml(score_test_hdf5=...)`, score-test row-data providers, `ldgm_score_test_hdf5_inventory()`, `ldgm_add_score_test_trait_group()`, `ldgm_remove_score_test_trait_groups()`, `ldgm_rename_score_test_trait_group()`, `ldgm_remove_score_test_traits()`, `ldgm_rename_score_test_trait()`, `ldgm_remove_score_test_hdf5_entries()`, `ldgm_rename_score_test_hdf5_entry()` | native `hdf5lib` writer implemented for row data, extra `/row_data` columns, `/traits/<trait>/{gradient,hessian}`, extra one-dimensional trait datasets, `/traits/<trait>/parameters/{parameters,jackknife_parameters}`, trait groups, callback-based score-test row-data providers, optional DuckDB-backed row-data adapters, a `show`-style HDF5 inventory helper, trait-group add/remove/rename helpers over `/groups`, trait remove/rename helpers over `/traits` with group-reference updates, and auto-detecting HDF5 entry helpers mirroring GraphLD `rm`/`mv` selection; prepared `ldgm_reml_inputs` now auto-fill metadata-backed row data for GraphREML score HDF5 writes and the native reader now accepts upstream singleton-second-dimension row-data datasets; shared-field GraphREML HDF5 parity is now checked on both the default metadata-driven tiny fixture and a synthetic two-block active-variant fixture; broader score-test CLI schema remains planned |
