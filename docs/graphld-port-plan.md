...
| GraphLD graphREML core/scheduler | `ldgm_reml_link()`, `ldgm_reml_surrogate_markers()`, `ldgm_reml_block()`, `ldgm_run_reml()`, `ldgm_reml_results()`, `ldgm_reml_convergence_results()`, `ldgm_write_reml_results()`, `ldgm_write_reml_outputs()` | initial serial core, S7/s7contract annotation inputs, GraphLD-style trust-region optimization, pseudo-jackknife summaries, GraphLD-style parameter/heritability/enrichment wide CSV helpers plus tall/convergence outputs and CLI-style output-family filenames, including alternate-output multi-fit append semantics, serial surrogate-marker handling, selected-view block support, GraphLD-style surrogate-map HDF5 input, max-chi-square block exclusion, and pinned upstream fixed-block plus multi-iteration optimizer-summary/history plus on-disk CSV-output-surface conformance implemented for currently shared fields; full CLI parity, upstream-scale jackknife conformance, and multiprocessing remain planned |
