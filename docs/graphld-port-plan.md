...
| GraphLD graphREML core/scheduler | `ldgm_reml_link()`, `ldgm_reml_surrogate_markers()`, `ldgm_reml_block()`, `ldgm_run_reml()`, `ldgm_reml_results()`, `ldgm_reml_convergence_results()`, `ldgm_write_reml_results()`, `ldgm_write_reml_outputs()` | initial serial core, S7/s7contract annotation inputs, GraphLD-style trust-region optimization, pseudo-jackknife summaries, GraphLD-style parameter/heritability/enrichment wide CSV helpers plus tall/convergence outputs and CLI-style output-family filenames, including alternate-output multi-fit append semantics, serial surrogate-marker handling, selected-view block support, zero-row placeholder block handling for jackknife/block-family bookkeeping, GraphLD-style surrogate-map HDF5 input, max-chi-square block exclusion, and pinned upstream fixed-block plus multi-iteration optimizer-summary/history plus on-disk single-fit, alternate-output multi-fit, and convergence/tall-existing-file CLI-surface conformance implemented for currently shared fields; full CLI parity, upstream-scale jackknife conformance, and multiprocessing remain planned |
| GraphLD graphREML score-test HDF5 output | `ldgm_write_score_test_hdf5()`, `ldgm_write_gene_score_hdf5()`, optional `ldgm_run_reml(score_test_hdf5=...)`, score-test row-data providers | native `hdf5lib` writer implemented for row data, extra `/row_data` columns, `/traits/<trait>/{gradient,hessian}`, extra one-dimensional trait datasets, `/traits/<trait>/parameters/{parameters,jackknife_parameters}`, trait groups, callback-based score-test row-data providers, and optional DuckDB-backed row-data adapters; broader score-test CLI schema remains planned |
