...
| GraphLD graphREML core/scheduler | `ldgm_reml_link()`, `ldgm_reml_surrogate_markers()`, `ldgm_reml_block()`, `ldgm_run_reml()` | initial serial core, S7/s7contract annotation inputs, GraphLD-style trust-region optimization, pseudo-jackknife summaries, serial surrogate-marker handling, selected-view block support, GraphLD-style surrogate-map HDF5 input, max-chi-square block exclusion, and pinned upstream fixed-block plus multi-iteration optimizer-summary conformance implemented; full CLI parity, upstream-scale jackknife conformance, and multiprocessing remain planned |
...
| `run_simulate()` workflow prototype | `ldgm_simulate()` | implemented (serial, metadata-driven); current pinned upstream comparison gate passes via `tools/generate-upstream-graphld-simulate.py` + `tools/check-upstream-graphld-simulate.R` across one-block, multi-block, and mixture scenarios, and local regression coverage now verifies multi-population metadata filters with per-row snplist frequency selection, while broader metadata/block scenarios and larger-fixture parity remain planned |
