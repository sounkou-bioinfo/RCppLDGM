...
| GraphLD `score_test.py` variant-annotation statistic | `ldgm_score_test()`, `ldgm_score_test_hdf5()`, `ldgm_score_test_meta()`, `ldgm_score_test_hdf5_meta()` | R-native score/jackknife/Z statistic, named gene-set inputs for variant- and gene-level HDF5 paths, and trait-group/all-traits HDF5 meta-analysis implemented against GraphLD's gradient + annotation path |
