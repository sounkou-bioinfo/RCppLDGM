...
| `run_simulate()` workflow prototype | `ldgm_simulate()` | implemented (serial, metadata-driven); current pinned upstream comparison gate passes via `tools/generate-upstream-graphld-simulate.py` + `tools/check-upstream-graphld-simulate.R`, while broader metadata/block scenarios and larger-fixture parity remain planned |
...
5. Define simulation and MATLAB-only workflow scope explicitly: keep simulation gate behavior explicit in
   `tools/check-upstream-graphld-simulate.R`, expand beyond the current pinned one-scenario pass, and add broader metadata/block fixture coverage.
