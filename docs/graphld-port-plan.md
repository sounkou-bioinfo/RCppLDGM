...
| GraphLD graphREML core/scheduler | `ldgm_reml_link()`, `ldgm_reml_surrogate_markers()`, `ldgm_reml_block()`, `ldgm_run_reml()`, `ldgm_reml_results()`, `ldgm_reml_convergence_results()`, `ldgm_write_reml_results()` | initial serial core, S7/s7contract annotation inputs, GraphLD-style trust-region optimization, pseudo-jackknife summaries, GraphLD-style wide/tall/convergence CSV helpers, serial surrogate-marker handling, selected-view block support, GraphLD-style surrogate-map HDF5 input, max-chi-square block exclusion, and pinned upstream fixed-block plus multi-iteration optimizer-summary/history plus CSV-output-surface conformance implemented for currently shared fields; full CLI parity, upstream-scale jackknife conformance, and multiprocessing remain planned |
