...
| GraphLD graphREML score-test HDF5 output | `ldgm_write_score_test_hdf5()`, `ldgm_write_gene_score_hdf5()`, optional `ldgm_run_reml(score_test_hdf5=...)`, score-test row-data providers | native `hdf5lib` writer implemented for row data, `/traits/<trait>/{gradient,hessian}`, `/traits/<trait>/parameters/{parameters,jackknife_parameters}`, trait groups, callback-based score-test row-data providers, and optional DuckDB-backed row-data adapters; broader score-test CLI schema remains planned |
