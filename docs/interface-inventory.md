...
| LDGM metadata CSV block catalog | GraphLD `metadata.csv`, `read_ldgm_metadata()`, BLUP/clump/reml CLI | `LdgmBlockCatalog`, `ldgm_block_catalog()`, `ldgm_load_block_catalog()`, `ldgm_load_ldgm()`, `ldgm_run_blup()`, `ldgm_run_clump()`, `ldgm_prepare_reml_inputs()`, `ldgm_run_reml()` list inputs | tinytest interfaces/BLUP/clump/reml; `tools/check-upstream-graphld-data.R` smoke; pinned BLUP/clump output conformance via `tools/check-upstream-graphld-blup-clump.R`; pinned GraphREML conformance via `tools/check-upstream-graphld-reml.R` now covers the original tiny metadata fixture plus a synthetic two-block active-variant fixture | Wire catalog objects into more GraphREML/block-manager paths and extend pinned output conformance beyond the current BLUP/clump/two-fixture REML slice |
...
| GraphREML model/method options | GraphLD `ModelOptions`, `MethodOptions`, `run_graphREML()` | `ldgm_reml_link()`, `ldgm_prepare_reml_inputs()`, `ldgm_reml_block()`, `ldgm_run_reml()`, `ldgm_reml_results()`, `ldgm_reml_convergence_results()`, `ldgm_write_reml_results()`, `ldgm_write_reml_outputs()` | tinytest reml; trust-region optimizer; pseudo-jackknife; GraphLD-style parameter/heritability/enrichment wide CSV helpers plus tall/convergence outputs and CLI-style output-family filenames, including alternate-output multi-fit append semantics; zero-row placeholder block handling for jackknife/block-family bookkeeping; metadata/provider staging with preserved full-row output mappings and node-scale Z preparation across duplicate-index merges; prepared-input score-test row-data auto-fill for GraphREML HDF5 outputs; HDF5/surrogate/max-chi-square slices; pinned GraphREML conformance via `tools/check-upstream-graphld-reml.R` now covers the default metadata-driven tiny fixture plus a synthetic two-block active-variant fixture, including per-block likelihood/per-variant-h2 checks, multi-iteration optimizer-summary/history, on-disk single-fit, alternate-output multi-fit, convergence/tall-existing-file, jackknife-summary, and shared-field score-test HDF5 comparisons | Full CLI parity, multiprocessing/block manager, upstream-scale jackknife conformance |
...
| GraphREML score-test HDF5 | GraphLD `_write_variant_data()`, `_write_trait_stats()`, `score_test_io.py`, `score_test` CLI `show`/`add-meta`/`rm`/`mv` | `LdgmScoreTestVariantData`, `LdgmScoreTestGeneData`, `ldgm_score_test_variant_data()`, `ldgm_score_test_variant_data_provider()`, `ldgm_duckdb_score_test_variant_data()`, `ldgm_score_test_gene_data()`, `ldgm_score_test_gene_data_provider()`, `ldgm_duckdb_score_test_gene_data()`, `ldgm_write_score_test_hdf5()`, `ldgm_write_gene_score_hdf5()`, `ldgm_read_score_test_hdf5()`, `ldgm_score_test_hdf5_inventory()`, `ldgm_add_score_test_trait_group()`, `ldgm_remove_score_test_trait_groups()`, `ldgm_rename_score_test_trait_group()`, `ldgm_remove_score_test_traits()`, `ldgm_rename_score_test_trait()`, `ldgm_remove_score_test_hdf5_entries()`, `ldgm_rename_score_test_hdf5_entry()`, `ldgm_score_test_hdf5_results()`, `ldgm_convert_variant_to_gene_scores()`, trait/group management and auto-detecting HDF5 entry helpers, extra row-data columns, and extra one-dimensional trait datasets | tinytest HDF5/score-test/interfaces/DuckDB; optional `tools/check-graphld-hdf5-python-interop.R`, which now also validates shared-field variant→gene conversion parity against upstream `convert_scores.py` plus tiny variant- and gene-level gene-set/pathway score-test parity; `tools/check-upstream-graphld-reml.R` now also checks shared-field GraphREML score-test HDF5 parity on both the metadata-driven tiny fixture and the synthetic two-block active-variant fixture | Broader CLI schema, h5py-required CI on systems with Python deps, and deeper out-of-memory writer paths beyond projected row-data callbacks |
