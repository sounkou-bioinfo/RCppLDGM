...
| GraphREML model/method options | GraphLD `ModelOptions`, `MethodOptions`, `run_graphREML()` | `ldgm_reml_link()`, `ldgm_reml_block()`, `ldgm_run_reml()` | tinytest reml; trust-region optimizer; pseudo-jackknife; HDF5/surrogate/max-chi-square slices; pinned fixed-block and one-step optimizer-summary conformance via `tools/check-upstream-graphld-reml.R` | Full CLI parity, multiprocessing/block manager, upstream-scale jackknife conformance |
...
4. Add upstream Python conformance for BLUP, clumping, GraphREML summaries beyond
   the current fixed-block and one-step optimizer-summary gates, and expand inverse-diagonal coverage beyond the current selected/full xdiag+hutchinson test-block gate.
