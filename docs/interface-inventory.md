...
| GraphREML model/method options | GraphLD `ModelOptions`, `MethodOptions`, `run_graphREML()` | `ldgm_reml_link()`, `ldgm_reml_block()`, `ldgm_run_reml()`, `ldgm_reml_results()`, `ldgm_reml_convergence_results()`, `ldgm_write_reml_results()`, `ldgm_write_reml_outputs()` | tinytest reml; trust-region optimizer; pseudo-jackknife; GraphLD-style parameter/heritability/enrichment wide CSV helpers plus tall/convergence outputs and CLI-style output-family filenames; HDF5/surrogate/max-chi-square slices; pinned fixed-block and multi-iteration optimizer-summary/history plus on-disk CSV-output-surface conformance via `tools/check-upstream-graphld-reml.R` | Full CLI parity, multiprocessing/block manager, upstream-scale jackknife conformance |
