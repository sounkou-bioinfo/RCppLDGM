...
| GraphLD score-test statistic | `ldgm_score_test()`, `ldgm_score_test_hdf5()` | HDF5 trait gradients + variant annotations + jackknife blocks | annotation score, jackknife SE, Z, log10 p-value | initial variant-annotation statistic implemented to mirror upstream `score_test.py`; gene-set conversion and meta-analysis remain planned |
