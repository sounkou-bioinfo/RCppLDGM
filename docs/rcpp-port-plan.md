...
| GraphLD score-test statistic | `ldgm_score_test()`, `ldgm_score_test_hdf5()`, `ldgm_score_test_meta()`, `ldgm_score_test_hdf5_meta()` | HDF5 trait gradients + variant annotations + jackknife blocks | annotation score, jackknife SE, Z, log10 p-value, inverse-jackknife-variance meta-analysis | variant-annotation statistic and caller-specified multi-trait meta-analysis implemented to mirror upstream `score_test.py`; gene-set conversion remains planned |
