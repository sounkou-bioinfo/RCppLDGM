...
| GraphLD `PrecisionOperator` | `ldgm_sparse_precision()`, `ldgm_precision_select()`, `ldgm_precision_update()`, `ldgm_precision_multiply()`, `ldgm_precision_solve()`, `ldgm_variant_solve()` | sparse precision matrix plus variant metadata | updated precision objects, matrix/vector results, duplicate-index variant solves | Rcpp sparse multiply plus Matrix/CHOLMOD solve, copy-return diagonal update, Schur-complement and chained-selection slices, and duplicate-index variant solve implemented |
...
| GraphLD graphREML core/scheduler | `ldgm_reml_link()`, `ldgm_reml_block()`, `ldgm_run_reml()` | precision blocks + Z scores + annotations | likelihood, gradient, Hessian, parameters, pseudo-jackknife summaries, heritability/enrichment, optional HDF5 score gradients | initial serial core, GraphLD-style pseudo-jackknife summaries, and native `hdf5lib` score-test gradient writer implemented; full CLI parity/upstream-scale jackknife conformance/surrogate/multiprocessing remains planned |
