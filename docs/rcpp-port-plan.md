...
| GraphLD graphREML core/scheduler | `ldgm_reml_link()`, `ldgm_reml_surrogate_markers()`, `ldgm_reml_block()`, `ldgm_run_reml()` | precision blocks + Z scores + annotations | likelihood, gradient, Hessian, parameters, pseudo-jackknife summaries, heritability/enrichment, optional HDF5 score gradients/corrections | initial serial core, S7/s7contract input interfaces, GraphLD-style pseudo-jackknife summaries, native `hdf5lib` score-test gradient/hessian/parameter writer, GraphLD-style surrogate-map HDF5 input, and max-chi-square block exclusion implemented; full CLI parity/upstream-scale jackknife conformance/multiprocessing remains planned |
| GraphLD score-test statistic and gene conversion | `ldgm_score_test()`, `ldgm_score_test_hdf5()`, `ldgm_score_test_meta()`, `ldgm_score_test_hdf5_meta()`, `ldgm_write_gene_score_hdf5()`, `ldgm_convert_variant_to_gene_scores()` | HDF5 trait gradients + variant/gene annotations + jackknife blocks + gene tables/GMT sets | annotation score, jackknife SE, Z, log10 p-value, inverse-jackknife-variance meta-analysis, gene-level score HDF5 | variant-annotation statistic, caller-specified multi-trait meta-analysis, gene-level HDF5 writing, and variant-to-gene score conversion implemented; full score-test CLI parity remains planned |
| GraphLD `run_simulate()` | `ldgm_simulate()` | metadata CSV + optional custom annotations | simulated summary statistics (`CHR`, `SNP`, `POS`, `A1`, `A2`, `Z`, `beta`, `beta_marginal`, `N`) | serial metadata-driven prototype implemented; conformance scaffold exists in `tools/check-upstream-graphld-simulate.R` but upstream runtime is currently blocked |
...
- **tskit adapter:** `ldgm_tree_tables_from_tskit()` uses the vendored tskit C
  API for `.trees` file paths by default, then hands canonical tables to the
  native Rcpp kernels. A `reticulate` backend remains available for live Python
  `tskit.TreeSequence` objects and for native-vs-Python extraction checks.
