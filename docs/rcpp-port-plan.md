...
| GraphLD `PrecisionOperator` | `ldgm_sparse_precision()`, `ldgm_precision_select()`, `ldgm_precision_update()`, `ldgm_precision_multiply()`, `ldgm_precision_solve()` | sparse precision matrix | updated precision objects and matrix/vector results | Rcpp sparse multiply plus Matrix/CHOLMOD solve, copy-return diagonal update, and Schur-complement slice implemented |
